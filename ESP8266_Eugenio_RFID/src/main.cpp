...
void publish_measurements(String t){
  ...
  payload.replace("schema_name", "leituras");
  payload.replace("field0", "tag");
  payload.replace("field0_value", t);
  Serial.printf("Publishing measurements with payload: %s\r\n", payload.c_str());
...
void extraiTAG (uint8_t t[], int n) {
  ...
  //Envia a tag
  publish_measurements(tag);
  client.loop();
  //Serial.println("LOOP FORÇADO");
...
void loop(){
  ...
  while ((bytesAvail = Serial.available()) > 0) {
      bytesIn = Serial.readBytes(buf, min(bytesAvail, sizeof(buf)));
    }
    /*for (int x = 0; x < bytesIn; x++) {
        Serial.print(String((char)buf[x], HEX));
        Serial.print("-");
    }*/
    
   for (int x = 0; x <= bytesIn-1 ; x++)
      {
        if (bytesIn >= 15)
        {
          if (buf[x] == 255 & buf[x+1] == 255 & buf[x+2] == 255) {
            extraiTAG(buf, x + 3);
            x += 14;
          }
        }
  ...
  else
  {
    ...
    if (millis() - last_mueasurement_publish_in_millis > 5000)
    {
      ...
      publish_measurements("E28011700000020C824EC1FF");
    }
  }*/
}
